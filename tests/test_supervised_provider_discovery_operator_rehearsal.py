"""Test actual-command supervised-provider discovery operator rehearsals."""
...
def test_discovery_operator_command_rehearsal_runs_every_scenario(
    tmp_path,
) -> None:
...
def test_discovery_operator_command_rehearsal_restart_returns_report(
    tmp_path,
) -> None:
    now = _now()
...
def test_discovery_operator_command_rehearsal_rejects_identity_reuse(
    tmp_path,
) -> None:
    _run(tmp_path)
    ...
    with pytest.raises(ValueError, match="bound to other inputs"):
        run_supervised_provider_discovery_operator_command_rehearsal(
            rehearsal_id="discovery-operator-command-rehearsal",
...
def test_discovery_operator_command_rehearsal_detects_changed_observation(
    tmp_path,
) -> None:
    report = _run(tmp_path)
...
def test_discovery_operator_command_rehearsal_detects_changed_record(
    tmp_path,
) -> None:
    report = _run(tmp_path)
...
def test_discovery_operator_command_rehearsal_detects_prohibited_artifact(
    tmp_path,
) -> None:
    _run(tmp_path)
...
def test_discovery_operator_command_rehearsal_requires_every_scenario(
    tmp_path,
) -> None:
    report = _run(tmp_path)
    ...
    with pytest.raises(ValueError, match="must include every scenario"):
        SupervisedProviderDiscoveryOperatorRehearsalReport.model_validate(
            payload
        )
...
def _run(
    ...
    return run_supervised_provider_discovery_operator_command_rehearsal(
        rehearsal_id="discovery-operator-command-rehearsal",
        output_root=root,
        quant_executable_path=_quant(),
...
def _report_path(root: Path) -> Path:
    return root / "reports" / "discovery-operator-command-rehearsal.json"
