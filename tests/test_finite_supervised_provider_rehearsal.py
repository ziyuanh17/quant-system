"""Test actual-command rehearsals for finite supervised-provider dry-runs."""
...
def test_finite_provider_command_rehearsal_runs_every_scenario(
    tmp_path,
) -> None:
...
def test_finite_provider_command_rehearsal_restart_returns_verified_report(
    tmp_path,
) -> None:
    now = _now()
...
def test_finite_provider_command_rehearsal_rejects_identity_reuse(
    tmp_path,
) -> None:
    _run(tmp_path)
    ...
    with pytest.raises(ValueError, match="bound to other inputs"):
        run_finite_supervised_provider_command_rehearsal(
            rehearsal_id="finite-command-rehearsal",
...
def test_finite_provider_command_rehearsal_detects_changed_observation(
    tmp_path,
) -> None:
    report = _run(tmp_path)
...
def test_finite_provider_command_rehearsal_detects_changed_loop_record(
    tmp_path,
) -> None:
    report = _run(tmp_path)
...
def test_finite_provider_command_rehearsal_detects_prohibited_artifact(
    tmp_path,
) -> None:
    _run(tmp_path)
...
def test_finite_provider_command_rehearsal_report_requires_every_scenario(
    tmp_path,
) -> None:
    report = _run(tmp_path)
    ...
    with pytest.raises(ValueError, match="must include every scenario"):
        FiniteSupervisedProviderRehearsalReport.model_validate(payload)


def _run(
    root: Path, *, now: datetime | None = None
) -> FiniteSupervisedProviderRehearsalReport:
    return run_finite_supervised_provider_command_rehearsal(
        rehearsal_id="finite-command-rehearsal",
        output_root=root,
        quant_executable_path=_quant(),
...
def _report_path(root: Path) -> Path:
    return root / "reports" / "finite-command-rehearsal.json"
