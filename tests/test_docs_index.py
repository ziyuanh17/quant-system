...
class TestCollectionCategorization:
    def test_incident_doc(self):
        assert (
            _categorize_collection(
                "actionable_paper_order_incident_2026-06-09.md"
            )
    ...
    def test_launchd_doc(self):
        assert (
            _categorize_collection("launchd_local_preflight.md")
            == "migration_and_scheduling"
        )

    def test_alpaca_doc(self):
        assert (
            _categorize_collection("alpaca_paper_adapter.md")
            == "safety_and_broker"
        )
...
class TestDocTypeCategorization:
    ...
    def test_runbook_doc_type(self):
        assert (
            _categorize_doc_type("alpaca_paper_smoke_runbook.md") == "runbook"
        )

    def test_design_doc_type(self):
        assert (
            _categorize_doc_type("alpaca_paper_adapter_design.md") == "design"
        )
...
class TestMarkdownRendering:
    ...
    def test_escapes_raw_html_and_code_blocks(self):
    ...
    def test_rejects_javascript_links(self):
        rendered = _simple_markdown_to_html("[bad](javascript:alert(1))")
