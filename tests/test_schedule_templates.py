...
def test_web_console_launchd_template_is_private_and_review_only() -> None:
    template_path = Path(
        "configs/launchd/com.quant-system.console.plist.example"
    )

    payload = plistlib.loads(template_path.read_bytes())

    assert payload["Label"] == "com.quant-system.console"
    assert payload["Disabled"] is True
    assert payload["WorkingDirectory"] == "/absolute/path/to/quant-system"
    assert payload["ProgramArguments"] == [
        "/bin/bash",
        "/absolute/path/to/quant-system/scripts/run_web_console.sh",
    ]
