...
def _status_from_health(
    ...
    return Status(
        ...
        severity="ok" if state in ("healthy", "not_configured", "skipped") else "error",
...
@router.get("/overview", response_model=OverviewResponse, dependencies=[Depends(require_api_key)])
async def overview() -> OverviewResponse:
    ...
    try:
        report = build_health_report(
            ...
            comparison_report_path=_DATA_DIR / "dry_run" / "comparison" / "latest.json",
            check_alpaca_paper=True,
            alpaca_paper_workflow_records_dir=_DATA_DIR / "workflows" / "alpaca-paper-refresh",
            alpaca_paper_reconciliation_report_path=_DATA_DIR / "live" / "reconciliation" / "latest.json",
         )
    except Exception:
        ...
        return OverviewResponse(
            system=OverviewSystem(
                server_status=_status_from_health("failed", "health_check", message="health check unavailable"),
                server_heartbeat=_status_from_health("failed", "health_check", message="heartbeat unavailable"),
            ...
            account_lanes=(
                ...
                connection=_status_from_health("unknown", "paper_state", message="unavailable"),
                permission="simulated",
                reconciliation=_status_from_health("unknown", "reconciliation", message="unavailable"),
                freshness=_status_from_health("unknown", "paper_state", message="unavailable"),
                 ),
                AccountLane(
                ...
                connection=_status_from_health("unknown", "broker_snapshot", message="unavailable"),
                permission="paper orders",
                reconciliation=_status_from_health("unknown", "reconciliation", message="unavailable"),
                freshness=_status_from_health("unknown", "broker_snapshot", message="unavailable"),
                 ),
                AccountLane(
                environment=Environment.REAL_MONEY,
                connection=_status_from_health("not_configured", "real_money", message="not configured"),
                permission="not_configured",
                reconciliation=_status_from_health("not_configured", "real_money", message="not configured"),
                freshness=_status_from_health("not_configured", "real_money", message="not configured"),
                 ),
             ),
            latest_workflows=[],
    ...
    account_lanes = (
        ...
        AccountLane(
            environment=Environment.DRY_RUN,
            connection=_status_from_health(report.comparison_status, "paper_state"),
            permission="record only",
            reconciliation=_status_from_health(report.comparison_status, "reconciliation"),
            freshness=_status_from_health(report.comparison_status, "paper_state"),
         ),
        AccountLane(
            environment=Environment.ALPACA_PAPER,
            connection=alpaca_connection,
            permission="paper orders" if report.alpaca_paper_workflow_status != "skipped" else "disabled",
    ...
    if report.latest_signal_path:
        latest_decisions[Environment.LOCAL_PAPER] = DecisionTrace(
            ...
            input_data=report.latest_signal_path,
            signal=report.latest_signal_action or "hold",
            signal_reason="",
            outcome=report.latest_signal_action or "hold",
