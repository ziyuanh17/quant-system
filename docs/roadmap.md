...
| 164 | Semantic-Target Alpaca Paper Testing Boundary v1 | In Review | Design the one-request semantic-target Alpaca paper testing boundary, allowing only paper-trade order/reconciliation API use and excluding launchd, scheduler, non-paper Alpaca behavior, and real-money trading. |

## Current Recommendation

Review **Semantic-Target Alpaca Paper Testing Boundary v1**, then implement a
source-level reviewed-request model and fake-client rehearsal for one semantic
target Alpaca paper test. Do not expose launchd, recurring scheduling,
